...
| CORE-015 | Logic in content violation | Locally implemented content contains conditional selection logic or any behavioral/architectural decision logic, or controller pushes presentation commands/state/mutations into locally implemented content. This includes deciding, deriving, branching, selecting, toggling, or computing structure, class/style/token, text, icon, visibility, handler, child output, platform primitive, representation, capability, orchestration, lifecycle, protocol routing, or state-driven behavior. Locally implemented content may only materialize a structurally static content shape and apply already-resolved primitive values from its owning controller access contract; controller must update its own state and request dirty/render/lifecycle refresh rather than command content to show, hide, update, configure, apply state, set class/style, or render with values. |
