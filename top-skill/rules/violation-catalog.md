...
| CORE-031 | Decomposed content access input | Controller receives, stores, or uses anything other than its own Content instance typed only as `IContentAccess`/target-equivalent. This includes receiving content command members as separate parameters, method bags, facade/adapters, closure bundles, concrete Content types, platform primitives, or objects assembled outside the controller/content construction boundary. Correctly named content command methods do not make a decomposed content-access bag a valid `IContentAccess`. |
