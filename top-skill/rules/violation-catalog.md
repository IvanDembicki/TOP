...
| CORE-019 | Content/View semantic injection | Content/View constructor or any public runtime/composition entrypoint receives data, callbacks, handlers, flags, state, stores, services, child components, slots, prebuilt fragments, platform child views, child view handles, child-output getter bundles, view-model objects, config/options/props-like objects, parameter bags, runtime argument sets, arbitrary props, a dummy zero-access object, or an externally assembled access bundle instead of exactly one narrow owner access interface implemented by the owning controller. |
