...
| CORE-032 | Context data injection | A TOP object is filled with data instead of attached to context. A node, locally implemented content object, connector, or black-box boundary constructor receives data packets, flags, callbacks, config/options/props-like objects, stores, services, child views, presentation values, visibility values, style values, text values, runtime state, handlers, or arbitrary additional arguments beyond its narrow contextual reference; or the owner pushes those values later through setter-style post-construction calls such as apply-config, set-data, set-visible, update-text, set-style, set-callbacks, or target-equivalent mutations. Objects must pull values through contextual contracts after attachment. |
