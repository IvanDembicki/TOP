...
- Do not store or use concrete Content classes from the controller when a separate content boundary exists. The controller must hold content through `IContentAccess`, which exposes only the allowed commands and opaque placement output.
